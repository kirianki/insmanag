...
const getCustomers = (search: string) => 
    api.get<{ results: SimpleCustomer[] }>('/customers/', { params: { search } });

const getPolicies = (customerId: string) => 
    api.get<{ results: SimplePolicy[] }>('/policies/', { params: { customer: customerId } });
...
export function FileClaimForm({ onSuccess }: { onSuccess: (newId: string) => void }) {
  ...
  const { data: customersResp, isLoading: isLoadingCustomers } = useQuery({
  ...
  const { data: policiesResp, isLoading: isLoadingPolicies } = useQuery({
    queryKey: ['policies', selectedCustomerId],
    queryFn: () => getPolicies(selectedCustomerId),
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 py-2">
        ...
        <FormField control={form.control} name="customer" render={({ field }) => (
          <FormItem className="flex flex-col">
            ...
            <SearchableCombobox 
              options={customersResp?.data.results?.map((c) => ({ 
                  value: c.id, 
        ...
        <FormField control={form.control} name="policy" render={({ field }) => (
          <FormItem className="flex flex-col">
            <FormLabel>Policy</FormLabel>
             <SearchableCombobox 
              options={policiesResp?.data.results?.map((p) => ({ 
                  value: p.id, 
